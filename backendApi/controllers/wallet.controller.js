...
// Razorpay is the source of truth when the customer closes the app after
// paying. This route must receive the raw request body for signature checks.
export const razorpayWebhook = async (req, res) => {
  try {
    ...
    // Only captured payments should credit a wallet.
    ...
    if (!razorpayPaymentId || !razorpayOrderId) {
      return res.status(400).json({
        success: false,
        message: "Webhook payment or order ID is missing",
      });
    }

    const order = await getRazorpayInstance().orders.fetch(razorpayOrderId);
    const userId = Number(order?.notes?.userId || 0);

    if (!userId) {
      return res.status(400).json({
        success: false,
        message: "Wallet user is missing from Razorpay order metadata",
      });
    }

    // Reuse the existing idempotent verification and wallet transaction.
    ...
    return res.status(responseStatus).json(responseBody || { success: true });
  } catch (error) {
