...
export const completeProfile = async (req, res) => {
  try {
    const { mobile, full_name, location, date_of_birth, time_of_birth, gender, acceptedTerms,latitude,longitude } = req.body;
console.log("Received profile data:", req.body);
	...
	latitude = ?
       WHERE id = ?`,
      [full_name, location, date_of_birth, time_of_birth || null, gender,acceptedTerms,longitude ,latitude , userId]
    );
...
export const updateBirthDetails = async (req, res) => {
  try {
    ...
    console.error("updateBirthDetails error:", error);

    return res.status(500).json({
      success: false,
      message: "Failed to update birth details",
    });
  }
};
export const getBirthProfile = async (req, res) => {
  try {
    const { userId } = req.params;
...
console.log(data)
    ...
    console.error("getBirthProfile error:", error);

    return res.status(500).json({
      success: false,
      message: "Failed to fetch birth profile",
    });
  }
};