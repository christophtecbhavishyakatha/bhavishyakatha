...
export const getNotice = async (req, res) => {
  try {
    ...
    console.log("User:", userId, "Wallet:", walletBalance);

    // Show notice only when wallet balance is below 30
    if (walletBalance >= 30) {
      return res.json({
        success: false,
        data: null,
    ...
    const notice = {
      image: "https://bhavishyakatha.in/express/image/notices/notice_v1.png",
      button_name: "Recharge Now",
      redirect_url: "/wallet",
    };
...
export const getAllAstrologers = async (req, res) => {
  try {
	...
	a.founding,
      ...
      ORDER BY
  CASE WHEN ast.status = 'online' THEN 0 ELSE 1 END,
  RAND()    `;
...
const [comments] = await db.query(commentsQuery, [id]);
  ...
  reply: c.reply || null,
  replyCreatedAt: c.reply_created_at || null,
})),
      },
    });
  } catch (error) {
  ...
  try {
    ...
    const [rows] = await db.query(
      ...
      SELECT
        ...
        ap.profile_photo AS astrologer_profile_photo,
