...
import { createCallRequestWithCustomerLock } from "../services/callRequestGuard.service.js";
...
const finalLatitude = latitude ?? lat ?? null;
const finalLongitude = longitude ?? long ?? null;

const parsedLatitude =
  finalLatitude !== null && finalLatitude !== ""
    ? Number(finalLatitude)
    : null;

const parsedLongitude =
  finalLongitude !== null && finalLongitude !== ""
...
const requestResult = await createCallRequestWithCustomerLock({
  pool,
  customerId: userId,
  insertSql: `INSERT INTO call_requests
   (
     astrologer_id,
     customer_id,
     call_type,
...
if (requestResult.userNotFound) {
  return res.json({ success: false, message: "User not found" });
}

if (requestResult.activeCall) {
  return res.status(409).json({
    success: false,
    code: "ACTIVE_CALL_EXISTS",
    message: "You already have an active call",
    activeCallId: requestResult.activeCallId,
  });
}
...
if (update_profile) {
  ...
  insertSql: `INSERT INTO call_requests
   (
     astrologer_id, customer_id, call_type, max_duration_sec,
     call_charge, platform_fee, status, full_name, date_of_birth,
     time_of_birth, location, latitude, longitude
   )
   VALUES (?, ?, 'video', ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?)`,
  insertParams: [
    astrologerId, userId, maxDurationSec, videoRate, platformFee,
    full_name || null, date_of_birth || null, time_of_birth || null,
    location || null, parsedLatitude, parsedLongitude,
  ],
});
...
const updateUserBirthProfile = async ({
  userId,
  full_name,
  date_of_birth,
  time_of_birth,
