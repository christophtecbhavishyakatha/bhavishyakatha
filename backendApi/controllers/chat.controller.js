...
export const initChatSession = async (req, res) => {
  try {
    ...
    // A chat must be accepted by the astrologer (and confirmed by the
    // customer's RingingScreen) before a session can be initialized.
    if (!["accepted", "ongoing"].includes(String(call.status).toLowerCase())) {
      return res.status(409).json({
        success: false,
        message: "Chat request is not accepted",
      });
    }
