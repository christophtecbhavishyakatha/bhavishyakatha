...
export default function HomeScreen() {
      ...
      await new Promise((resolve) => {
        Alert.alert(
          ...
          [
            {
              text: "Cancel",
              onPress: () => { router.replace("/login"); resolve(false); },
            },
            {
              text: "Open Settings",
              onPress: async () => {
  ...
  const fetchCalls = async (user_id: string, showLoading = false) => {
    try {
      ...
      const res = await fetch(`${API_BASE}/astrologer/call-requests`, {
        method: "POST",
  ...
  const updateStatusAPI = async (newStatus = status, newAudio = audio, newVideo = video, newChat = chat) => {
    try {
      const res = await fetch(`${API_BASE}/astrologer/status/update`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: userId, status: newStatus, audio: newAudio, video: newVideo, chat: newChat }),
      });
  ...
  const saveAvailability = async (uid: string, values: { audio: boolean; video: boolean; chat: boolean }) => {
  ...
  const updateAll = async (
    ...
    try {
      ...
      const success = await updateStatusAPI(newStatus, newAudio, newVideo, newChat);
  ...
  const saveBio = async () => {
    try {
      ...
      const json = await res.json();
      if (json.success) { Alert.alert("Success", "Bio updated"); setBioModal(false); }
    } catch (err) {
      console.error(err);
    } finally {
      setBioLoading(false);
  ...
  const acceptAudioCall = async (customerId: string, id: string, fullName: string ,dateOfBirth:string, timeOfBirth:string, birthLocation: string) => {
    setLoading(true);
    try {
      ...
      if (json.success) {
        ...
        router.replace({ pathname: "/audiocall" as any, params: { customerId, userId, fullName, id , dateOfBirth, timeOfBirth : timeOfBirth ??"", birthLocation } });
  ...
  const acceptVideoCall = async (customerId: string, id: string, fullName: string, dateOfBirth:string, timeOfBirth:string, birthLocation: string) => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/astrologer/accept-call`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ astrologer_id: userId, customer_id: customerId, call_type: "video", id }),
      });
      const json = await res.json();
      if (json.success) {
        await fetchCalls(userId, false);
        router.replace({ pathname: "/videocall" as any, params: { customerId, userId, fullName, id, dateOfBirth, timeOfBirth, birthLocation } });
      }
  ...
  const acceptChat = async (customerId: string, id: string, fullName: string, dateOfBirth:string, timeOfBirth:string, birthLocation: string) => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/astrologer/accept-call`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ astrologer_id: userId, customer_id: customerId, call_type: "chat", id }),
      });
      const json = await res.json();
      if (json.success) {
        await fetchCalls(userId, false);
        router.replace({ pathname: "/chatbox" as any, params: { customerId, userId, fullName, id , dateOfBirth, timeOfBirth : timeOfBirth ??"", birthLocation} });
  ...
  const rejectCall = async (customerId: string, id: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/astrologer/reject-call`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ astrologer_id: userId, customer_id: customerId, id }),
      });
      const json = await res.json();
      ...
      <SafeAreaView style={styles.safeArea} edges={["top"]}>
        <LinearGradient colors={["#1A0533", "#2D0A5E", "#1A0533"]} style={styles.fullFlex}>
          <View style={styles.center}>
            ...
            <ActivityIndicator size="large" color="#F5C842" style={{ marginTop: 16 }} />
  ...
  const statusConfig = {
    online:  { label: "Online",  activeCard: styles.statusOnlineActive,  dot: styles.dotOnline,  activeText: styles.statusTextOnline },
    offline: { label: "Offline", activeCard: styles.statusOfflineActive, dot: styles.dotOffline, activeText: styles.statusTextOffline },
    busy:    { label: "Busy",    activeCard: styles.statusBusyActive,    dot: styles.dotBusy,    activeText: styles.statusTextBusy },
  };
          ...
          <View style={styles.headerBg}>
            ...
            <View style={styles.headerRow}>
              <View style={styles.headerLeft}>
                ...
                <Text style={styles.headerName}>{name}  (<Feather name="star" size={18} color="#FFD700" /> {rank})</Text>
...
<Text style={styles.categoryText}>
  {category.charAt(0).toUpperCase() + category.slice(1)}
              ...
              <View style={[styles.avatarRing, founding === 1 && { right: "12%" }]}>
                <LinearGradient colors={["#7C3AED", "#4C1D95"]} style={styles.avatarCircle}>
      ...
      FOUNDING{"\n"}ASTROLOGER
          ...
          >
            ...
            <View style={styles.walletInner}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 0,justifyContent: "space-between" }}>
              <Text style={styles.walletLabel}>💰 Wallet Balance</Text>
               <TouchableOpacity style={styles.txnBtn} onPress={() => router.push("/withdrawalHistory")}>
                <Text style={styles.txnBtnText}>Bank Transfer History →</Text>
              </TouchableOpacity>
              </View>
              <Text style={styles.walletAmount}>₹ {wallet.toLocaleString()}</Text> 
              
              <View style={{flexDirection: "row", alignItems: "center", justifyContent: "space-between", }}>
                <TouchableOpacity style={styles.txnBtn} onPress={() => router.push("/chatList")}>
                <Text style={styles.txnBtnText}>Chat History →</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.txnBtn} onPress={() => router.push("/transactions")}>
          ...
          <View style={styles.quickRow}>
            <TouchableOpacity
              ...
              onPress={() => { setBioModal(true); fetchBio(); }}
            >
              <Text style={styles.quickIcon}>📝</Text>
              <Text style={styles.quickLabel}>Update Bio</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.quickBtn} onPress={() => router.push("/photos")}>
          ...
          <Modal visible={bioModal} animationType="slide" transparent>
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalCard}>
                ...
                <Text style={styles.modalSub}>Tell seekers about your expertise</Text>
                {bioLoading && (
                ...
                />
                <TouchableOpacity style={styles.modalSaveBtn} onPress={saveBio}>
                  <LinearGradient
                ...
                <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setBioModal(false)}>
              ...
              {(["online", "offline", "busy"] as const).map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[styles.statusLabel, isActive && cfg.activeText]}>
                      {cfg.label}
                    </Text>
                  </TouchableOpacity>
                );
          ...
          <View style={styles.section}>
            ...
            <View style={styles.availRow}>
              <AvailChip label="Audio Call" emoji="🔊" value={audio} onPress={() => updateAll(status, !audio, video, chat)} />
              <AvailChip label="Video Call" emoji="🎥" value={video} onPress={() => updateAll(status, audio, !video, chat)} />
              <AvailChip label="Chat"       emoji="💬" value={chat}  onPress={() => updateAll(status, audio, video, !chat)} />
            </View>
          </View>
                ...
                >
                  ...
                  <View style={styles.callCardTop}>
                    <View style={styles.callAvatarRing}>
                      <LinearGradient colors={["#7C3AED", "#4C1D95"]} style={styles.callAvatar}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.callMeta}>
                        🎂 {item.date_of_birth}
                        {item.time_of_birth ? `  🕐 ${item.time_of_birth}` : ""}
                      </Text>
                      {item.location ? (
                  ...
                  <View style={styles.callActions}>
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        if (item.call_type === "audio") acceptAudioCall(item.customer_id, item.id, item.full_name, item.date_of_birth, item.time_of_birth,item.location);
                        else if (item.call_type === "video") acceptVideoCall(item.customer_id, item.id, item.full_name, item.date_of_birth, item.time_of_birth,item.location);
                        else if (item.call_type === "chat") acceptChat(item.customer_id, item.id, item.full_name, item.date_of_birth, item.time_of_birth,item.location);
                      }}
                    >
                      <LinearGradient
                        colors={["#7C3AED", "#5B21B6"]}
...
/* ---------- AVAIL CHIP ---------- */
function AvailChip({ label, emoji, value, onPress }: {
  label: string; emoji: string; value: boolean; onPress: () => void;
}) {
  return (
    <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  loadingText: { marginTop: 10, color: "#C4B5FD", fontWeight: "600", fontSize: 13, letterSpacing: 1 },
  ...
  headerSub: { fontSize: 9, color: "#F5C842", fontWeight: "700", letterSpacing: 2, marginBottom: 2 },
  headerName: { fontSize: 20, fontWeight: "800", color: "#F3F0FF", letterSpacing: 0.2, flexShrink: 1 },
  dpBadge: {
  ...
  avatarCircle: { flex: 1, borderRadius: 20, justifyContent: "center", alignItems: "center", },
  avatarText: { fontSize: 18, fontWeight: "800", color: "#F3F0FF" },
  refreshRow: { flexDirection: "row", alignItems: "center", paddingHorizontal: 14, paddingBottom: 8, gap: 4 },
...
ribbonText: {
  ...
  walletOrb: {
    ...
    backgroundColor: "rgba(245,200,66,0.07)", top: -20, right: -10,
  },
  walletOrb2: {
    position: "absolute", width: 65, height: 65, borderRadius: 32,
    backgroundColor: "rgba(124,58,237,0.08)", bottom: -15, left: 20,
  },
  walletInner: { paddingHorizontal: 16, paddingVertical: 12 },
  walletLabel: { fontSize: 10, color: "#F5C842", fontWeight: "700", letterSpacing: 1.5, marginBottom: 2 },
  walletAmount: { fontSize: 28, fontWeight: "800", color: "#F3F0FF", letterSpacing: 0.3 },
  txnBtn: {
  ...
  /* Quick Actions */
  quickRow: { flexDirection: "row", flexWrap: "wrap", marginHorizontal: 14, marginBottom: 10, gap: 10 },
  quickBtn: {
    flex: 1, minWidth: 96, backgroundColor: "rgba(255,255,255,0.06)",
    borderRadius: 14, paddingVertical: 10,
    alignItems: "center", borderWidth: 1.5,
    borderColor: "rgba(245,200,66,0.25)", gap: 3,
  },
  quickIcon: { fontSize: 18 },
  quickLabel: { fontSize: 11, fontWeight: "700", color: "#DDD6FE" },

  /* Modal */
  modalOverlay: { flex: 1, backgroundColor: "rgba(10,0,30,0.7)", justifyContent: "flex-end" },
  modalCard: {
  ...
  modalTitle: { fontSize: 20, fontWeight: "800", color: "#F3F0FF", marginBottom: 4, paddingHorizontal: 24 },
  modalSub: { fontSize: 13, color: "#9E7DC8", marginBottom: 18, paddingHorizontal: 24 },
  bioInput: {
    borderWidth: 1.5, borderColor: "rgba(124,58,237,0.4)",
    borderRadius: 16, padding: 16, minHeight: 130,
    textAlignVertical: "top", fontSize: 15, color: "#F3F0FF",
    backgroundColor: "rgba(255,255,255,0.05)",
    marginBottom: 18, marginHorizontal: 24,
  },
  modalSaveBtn: { borderRadius: 16, overflow: "hidden", marginHorizontal: 24, marginBottom: 12 },
  modalSaveBtnGradient: { paddingVertical: 15, alignItems: "center" },
  modalSaveBtnText: { color: "#FFFFFF", fontWeight: "800", fontSize: 15, letterSpacing: 0.5 },
  modalCloseBtn: {
    marginHorizontal: 24, paddingVertical: 13, borderRadius: 16,
    alignItems: "center", borderWidth: 1.5,
  ...
  sectionTitle: { fontSize: 13, fontWeight: "800", color: "#F3F0FF", flex: 1, letterSpacing: 0.3 },
  countBadge: {
  ...
  statusCard: {
    flex: 1, minWidth: 88, paddingVertical: 9,
    borderRadius: 12,
    backgroundColor: "rgba(255,255,255,0.06)",
    alignItems: "center", borderWidth: 1.5,
  ...
  statusOnlineActive:  { backgroundColor: "rgba(22,163,74,0.15)",  borderColor: "#16A34A" },
  statusOfflineActive: { backgroundColor: "rgba(156,163,175,0.15)", borderColor: "#9CA3AF" },
  statusBusyActive:    { backgroundColor: "rgba(239,68,68,0.15)",   borderColor: "#EF4444" },
  ...
  statusLabel: { fontSize: 11, fontWeight: "700", color: "rgba(255,255,255,0.4)" },
  ...
  chip: {
    flex: 1, minWidth: 86, borderRadius: 12, borderWidth: 1.5,
    borderColor: "rgba(124,58,237,0.3)",
  ...
  chipLabel:       { fontSize: 10, fontWeight: "700", color: "#C4B5FD", textAlign: "center" },
  chipLabelActive: { fontSize: 10, fontWeight: "700", color: "#FFFFFF",  textAlign: "center" },
  chipCheck: { fontSize: 10, color: "#F5C842", fontWeight: "800" },

  /* Call Cards */
  callCard: {
    borderRadius: 18, marginBottom: 12, overflow: "hidden",
    borderWidth: 1.5, borderColor: "rgba(245,200,66,0.3)",
    shadowColor: "#7C3AED",
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.4, shadowRadius: 10, elevation: 6,
  },
  callCardTopBar: { height: 2 },
  callCardTop: { flexDirection: "row", gap: 12, padding: 14, alignItems: "flex-start" },
  callAvatarRing: {
    width: 48, height: 48, borderRadius: 24,
    borderWidth: 2, borderColor: "#F5C842", padding: 2,
    shadowColor: "#F5C842",
    shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 0.3, shadowRadius: 6, elevation: 4,
  },
  callAvatar: { flex: 1, borderRadius: 22, justifyContent: "center", alignItems: "center" },
  callAvatarText: { fontSize: 18, fontWeight: "800", color: "#F3F0FF" },
  callNameRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 4 },
  callName: { fontSize: 15, fontWeight: "800", color: "#F3F0FF", flex: 1 },
  callMeta: { fontSize: 12, color: "#C4B5FD", fontWeight: "600", marginBottom: 2, lineHeight: 18 },
  callLocation: { fontSize: 11, color: "#9E7DC8", fontWeight: "500" },
  callDivider: { height: 1, backgroundColor: "rgba(255,255,255,0.08)", marginHorizontal: 14 },
  ...
  acceptBtnText: { color: "#FFFFFF", fontWeight: "800", fontSize: 14, letterSpacing: 0.2 },
  rejectBtn: {
