...
useFocusEffect(
 ...
 const fetchTickets = async () => {
  ...
  `${API_BASE}/tickets/${userId}?userType=user`
);
        ...
        <View style={styles.form}>
              ...
              {ISSUE_TYPES.map((issue) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    form.issueType === issue.value && styles.issueTypeCardActive
                  ]}
                  onPress={() => setForm({ ...form, issueType: issue.value })}
                >
                  <Text style={[
                    styles.issueTypeText,
                    form.issueType === issue.value && styles.issueTypeTextActive
                  ]}>
                    {issue.label}
                  </Text>
                </TouchableOpacity>
          ...
          <View style={styles.inputWrap}>
            <Text style={styles.label}>
              Describe Your Issue * 
              <Text style={styles.labelHint}> ({form.details.length}/500)</Text>
            </Text>
          <TextInput
          ...
          <View style={styles.uploadSection}>
            ...
            {form.screenshot ? (
              <View style={styles.previewContainer}>
                <Image
                  source={{ uri: `data:image/jpeg;base64,${form.screenshot}` }}
        ...
        {tickets.length === 0 ? (
          ...
          tickets.map((ticket) => (
            <View key={ticket.id} style={styles.ticketCard}>
              <View style={styles.ticketHeader}>
                <View style={styles.ticketType}>
                  <Text style={styles.ticketTypeEmoji}>
                    {ISSUE_TYPES.find(t => t.value === ticket.issue_type)?.label.split(' ')[0] || '❓'}
                  </Text>
                  <Text style={styles.ticketTypeText}>
                    {ISSUE_TYPES.find(t => t.value === ticket.issue_type)?.label.replace(/^.+?\s/, '') || 'Other'}
                  </Text>
                </View>
              ...
              {ticket.screenshot ? (
                <Image
                  source={{ uri: `data:image/jpeg;base64,${ticket.screenshot}` }}
